...
// 'T' (1B = FL_*): 축 A HW 글리치 트리거 지점. 0=전체 c·s~+y, FL_*=그 연산 직전·직후에만 트리거.
uint8_t cmd_trigline(uint8_t *in, uint8_t len){ (void)len;
    g_trig_line = (in[0] == 0) ? -1 : (int)in[0];
    simpleserial_put('r', 1, (uint8_t[]){(uint8_t)in[0]}); return 0x00; }
...
//           → 'r' 4B = cnt. 무결함이면 cnt==K(0x00010000); cnt!=K 이면 명령어 스킵 성공(장비 검증).
uint8_t cmd_calib(uint8_t *in, uint8_t len){ (void)in; (void)len;
    volatile uint32_t cnt = 0; const uint32_t K = 0x00010000u;
    trigger_high();
    for (volatile uint32_t z = 0; z < K; z++) cnt++;
    trigger_low();
    uint8_t b[4]; for (int i = 0; i < 4; i++) b[i] = (uint8_t)(cnt >> (8*i));
    simpleserial_put('r', 4, b); return 0x00; }
#endif
...
int main(void){
    ...
    simpleserial_addcmd('T', 1,  cmd_trigline);// [FL_*] HW 글리치 트리거 지점 선택 (축 A). 0=전체
...
#endif
#ifdef AXISA_JIG
    simpleserial_addcmd('J', 1,  cmd_jig);     // 축 A fast-jig: 0=prime, 1=fire(+y 재실행)
    simpleserial_addcmd('C', 0,  cmd_calib);   // 글리치 캘리브레이션(알려진 명령어 스킵 성공률)
#endif
