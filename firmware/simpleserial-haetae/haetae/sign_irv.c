...
#include "symmetric.h"
#ifdef CW_TARGET
#include "hal.h"   /* ChipWhisperer: trigger_high()/trigger_low() */
#endif

#include "fips202.h"

/* ===== HAETAE-IRV countermeasure helpers ===== */
static uint64_t haetae_sk_checksum(const uint8_t *sk){
  uint8_t out[8];
  shake256(out,8,sk+HAETAE_CRYPTO_PUBLICKEYBYTES,
           HAETAE_CRYPTO_SECRETKEYBYTES-HAETAE_CRYPTO_PUBLICKEYBYTES);
  uint64_t r=0; for(int i=0;i<8;i++) r|=((uint64_t)out[i])<<(8*i); return r;
}
/* Branchless infective masking: if delta!=0 the signature is scrambled with a
   PRF stream keyed by the secret seed, the residual delta, and the per-signature
   message hash mu; if delta==0 it is left untouched. Binding mu gives mask
   freshness: an identical delta on different messages yields a different mask,
   so two infected outputs never XOR-cancel the mask. */
static void haetae_infective_mask(uint8_t *sig,size_t siglen,uint64_t delta,
                                  const uint8_t *sk,const uint8_t *mu){
  uint8_t seed[HAETAE_SEEDBYTES+8+HAETAE_CRHBYTES]; uint8_t stream[HAETAE_CRYPTO_BYTES];
  uint8_t factor=(uint8_t)(0u-(uint8_t)((delta|(0u-delta))>>63));
  for(int i=0;i<HAETAE_SEEDBYTES;i++) seed[i]=sk[HAETAE_CRYPTO_SECRETKEYBYTES-HAETAE_SEEDBYTES+i];
  for(int i=0;i<8;i++) seed[HAETAE_SEEDBYTES+i]=(uint8_t)(delta>>(8*i));
  for(int i=0;i<HAETAE_CRHBYTES;i++) seed[HAETAE_SEEDBYTES+8+i]=mu[i];
  shake256(stream,siglen,seed,HAETAE_SEEDBYTES+8+HAETAE_CRHBYTES);
  for(size_t i=0;i<siglen;i++) sig[i]^=(uint8_t)(stream[i]&factor);
}
...
int crypto_sign_signature_internal(uint8_t *sig, size_t *siglen,
  ...
  uint64_t irv_ks0 = haetae_sk_checksum(sk);
  uint64_t irv_delta = 0;
...
reject:
...
#ifdef CW_TARGET
  trigger_high();   /* <-- glitch target: skip/abort this multiply */
#endif
  ...
  /* ============ HAETAE-IRV: redundant verification of the sensitive
     multiply c*s, independent re-check of the signing norm bounds, and a
     secret-key integrity checksum. The residual irv_delta is 0 iff the
     signing computation was fault-free; it is later folded into the output
     by branchless infective masking. ============ */
  {
    polyvecl cs1b;
    polyveck cs2b;
    unsigned int ii;
    cs1b.vec[0] = c;
    for (ii = 1; ii < HAETAE_L; ++ii) {
      poly_pointwise_montgomery(&cs1b.vec[ii], &chat, &s1.vec[ii - 1]);
      poly_invntt_tomont(&cs1b.vec[ii]);
    }
    polyveck_poly_pointwise_montgomery(&cs2b, &s2, &chat);
    polyveck_invntt_tomont(&cs2b);
    polyvecl_cneg(&cs1b, b & 1);
    polyveck_cneg(&cs2b, b & 1);
    for (ii = 0; ii < HAETAE_L; ++ii)
      for (int jj = 0; jj < HAETAE_N; ++jj)
        irv_delta |= (uint64_t)(uint32_t)(cs1.vec[ii].coeffs[jj] ^ cs1b.vec[ii].coeffs[jj]);
    for (ii = 0; ii < HAETAE_K; ++ii)
      for (int jj = 0; jj < HAETAE_N; ++jj)
        irv_delta |= (uint64_t)(uint32_t)(cs2.vec[ii].coeffs[jj] ^ cs2b.vec[ii].coeffs[jj]);
    /* signing-bound (B1') re-check */
    irv_delta |= (((uint64_t)HAETAE_B1SQ * HAETAE_LN * HAETAE_LN -
                   polyfixveclk_sqnorm2(&z1, &z2)) >> 63) & 1ULL;
    /* bimodal-bound (B0') re-check */
    { uint64_t r2 = (polyfixveclk_sqnorm2(&z1tmp, &z2tmp) -
                     (uint64_t)HAETAE_B0SQ * HAETAE_LN * HAETAE_LN) >> 63;
      r2 &= 1; r2 &= (b & 0x2) >> 1; irv_delta |= r2; }
    /* secret-key integrity */
    irv_delta |= (uint64_t)(haetae_sk_checksum(sk) != irv_ks0);
  }
  ...
  haetae_infective_mask(sig, HAETAE_CRYPTO_BYTES, irv_delta, sk, mu);
