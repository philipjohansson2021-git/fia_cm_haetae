...
enum { CM_BASE=0, CM_IRV, CM_LEEHA, CM_IRI };   /* CM_IRI = 경량 witness 변형 */
#define FI_MAX_ITER 4000
int g_fp=FP_NONE, g_fm=FM_NONE, g_cm=CM_BASE;
int g_irv_detected=0, g_leeha_detected=0, g_iter=0;
int g_iri_detected=0;   /* witness(CM_IRI) 탐지 플래그(=실제로는 무분기 감염 마스크 구동) */
...
/* ---- IRI witness helpers (경량·정확) ---- */
/* 임의 정수를 mod q 정규대표 [0,Q) 로. (표현 무관 비교용) */
static int32_t iri_canon(int64_t a){ int64_t r=a%HAETAE_Q; if(r<0)r+=HAETAE_Q; return (int32_t)r; }
/* R_q=Z_q[x]/(x^N+1) 에서 곱 c*s 의 k번째 계수(negacyclic), 정규대표 [0,Q).
   c,s 는 정규영역(작은) 계수. 방출 cs 는 NTT 기반 표현이라 iri_canon 후 비교한다. */
static int32_t iri_prod_coeff(const int32_t *c,const int32_t *s,int k){
  int64_t acc=0; int j;
  for(j=0;j<=k;j++)         acc += (int64_t)c[j]*s[k-j];
  for(j=k+1;j<HAETAE_N;j++) acc -= (int64_t)c[j]*s[HAETAE_N+k-j];
  return iri_canon(acc);
}
...
int sign_fi(uint8_t *sig, size_t *siglen,
  ...
  polyvecm s1, s1_normal;   /* s1_normal = NTT 전 사본(IRI witness T1 부분재계산용) */
  ...
  memcpy(&s1_normal, &s1, sizeof(s1_normal));  /* IRI: NTT 전 s1 사본 저장(부분재계산 입력) */
  polyvecm_ntt(&s1);
  polyveck_ntt(&s2);
...
reject:
  ...
  /* ===== HAETAE-IRI (경량 witness) : 응답연산 전용, wide·무분기 잔차 ===== *
   * 현 IRV M1(전체 c*s 재NTT곱+전계수 비교)과 원리 동일하나 구현이 다름:
   *   T1 = c*s 부분계수(m개) 스쿨북 재계산(전체 NTT 재계산 대신)
   *   T2 = 기존 cs 재사용 재덧셈(곱셈 재계산 없음)
   *   RB = 서명경계 B1+B0 재검사 / key = 체크섬
   * 잔차는 δ에 `|= (a^b)` 로 wide·무분기 누적(1비트 아님, if 아님).            */
  if (g_cm==CM_IRI) {
    uint64_t delta = 0;
    const int IRI_M = 4;   /* 검사 계수 개수(부분재계산): 비용↔커버리지 트레이드(튜닝가능) */

    /* --- witness-T1: c*s1 부분계수 재계산 vs 방출 cs1 (mod q 정규화 후 XOR) --- */
    for (unsigned int ii = 1; ii < HAETAE_L; ++ii) {
      const int32_t *sc = s1_normal.vec[ii-1].coeffs;
      for (int t = 0; t < IRI_M; ++t) {
        int k = (t * 67) % HAETAE_N;                        /* 흩뿌린 인덱스(0,67,134,201) */
        int32_t P = iri_prod_coeff(c.coeffs, sc, k);        /* 참 곱 계수 (canonical) */
        int32_t expect = (b & 1) ? (P ? (int32_t)(HAETAE_Q - P) : 0) : P; /* (-1)^b 적용 */
        int32_t emit = iri_canon((int64_t)cs1.vec[ii].coeffs[k]);         /* 방출값 canonical */
        delta |= (uint32_t)(expect ^ emit);                 /* wide·무분기 */
      }
    }
    /* --- witness-T2: cs1 재사용 재덧셈(z1b=y1+LN*cs1) vs 방출 z1 (곱셈 재계산 없음) --- */
    { polyfixvecl z1b; polyfixvecl_add(&z1b, &y1, &cs1);
      for (unsigned int ii = 0; ii < HAETAE_L; ++ii)
        for (int k = 0; k < HAETAE_N; k++)
          delta |= (uint32_t)(z1.vec[ii].coeffs[k] ^ z1b.vec[ii].coeffs[k]); }
    /* --- witness-RB: 서명경계 B1+B0 재검사 잔차(거부우회) --- */
    { uint64_t r1 = ((uint64_t)HAETAE_B1SQ*HAETAE_LN*HAETAE_LN - polyfixveclk_sqnorm2(&z1,&z2)) >> 63;
      ...
      r2 &= (b & 0x2) >> 1;
      delta |= (r1 & 1); delta |= (r2 & 1); }
    /* --- witness-key: 비밀키 체크섬(영속 변조) --- */
    if (g_have_golden_ck && g_cur_ck != g_golden_ck) delta |= 1;

    if (delta) g_iri_detected = 1;  /* 실제로는 무분기 감염 마스크 구동; 여기선 탐지 플래그 */
  }
