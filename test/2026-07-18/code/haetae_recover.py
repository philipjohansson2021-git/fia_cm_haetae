...
# ── 부분누설(다중결함) 누적 복구 ──────────────────────────────────────────────
# 실제 full-HAETAE 에서 단일 클럭글리치는 +y(1024계수) 루프의 '일부 계수'만 y-add 를 스킵한다.
# 그 계수는 z1[i] = (-1)^b · LN · cs1[i] 로 남아 LN 의 배수가 된다(clean-skip 계수).
# 결정론 키라 매 서명의 정상 z1(z1_normal)이 동일 → ext 를 +y 루프 전체에 스캔하며
# 서로 다른 계수의 clean-skip 을 모아 c·s1 전블록을 채우면 s1 을 복원한다(현실적 다중결함 T2).

def partial_coeffs(z1, z1_normal=None):
    """faulted z1 에서 clean-skip 계수 {index: (-1)^b·cs1[i]} 딕셔너리.
    판정: z1[i] 가 LN 의 배수(그 계수는 y 가 더해지지 않음).
    z1_normal 주면 '정상값과 다름'을 추가 조건 → y1[i] 가 우연히 LN 배수인 오탐 제거."""
    out = {}
    if not any(int(z1[i]) != 0 for i in range(len(z1))):
        return out                         # z1 전부 0 = 크래시/퇴화 읽기 → clean-skip 아님(거짓양성 방지)
    for i in range(len(z1)):
        if z1[i] % LN != 0:
            continue
        if z1[i] == 0:
            continue                       # 계수 0 = 정보 없음(cs1[i]=0 은 복원에 무의미) → 오탐 제거
        if z1_normal is not None and z1[i] == z1_normal[i]:
            continue                       # y 그대로(미스킵) 또는 y[i]=0 → 정보 없음
        out[i] = z1[i] // LN               # = (-1)^b · cs1[i]  (block0=c, block1-3=c·s1)
    return out

class PartialAccumulator:
    """여러 faulted z1 에서 clean-skip 계수를 누적 → c·s1 전블록 완성 시 s1 복원."""
    def __init__(self, c, z1_normal=None):
        self.c = list(c)
        self.z1n = list(z1_normal) if z1_normal is not None else None
        self.known = [None] * 1024         # (-1)^b·cs1 (coeff domain): block0=c, 1-3=c·s1[0..2]
        self.sign = None
        self.n_add = 0

    def coverage(self):                    # [c, s1[0], s1[1], s1[2]] 각 블록의 확보 계수 수(/256)
        return [sum(self.known[b*256+k] is not None for k in range(256)) for b in range(4)]

    def add(self, z1):
        """faulted z1 한 개 누적. 반환: (이번에 새로 얻은 계수 수, 현재 coverage)."""
        new = 0
        for i, v in partial_coeffs(z1, self.z1n).items():
            if self.known[i] is None:
                self.known[i] = v; new += 1
        self.n_add += 1
        if self.sign is None:              # block0 = (-1)^b·c 로 전역 부호 결정
            for i in range(256):
                if self.known[i] is not None and self.c[i] != 0:
                    if   self.known[i] ==  self.c[i]: self.sign = +1; break
                    elif self.known[i] == -self.c[i]: self.sign = -1; break
        return new, self.coverage()

    def recover(self, true_s1ntt=None):
        """256/256 완성된 비밀블록에 대해 s1 복원. 반환 dict:
        coverage, s1ntt(블록별, 미완성=None), (3블록 완성+참s1 주면) agreement/sign."""
        chat = ntt(self.c[:]); chatinv = [_inv(v) for v in chat]
        recs = []
        for b in range(1, 4):
            blk = [self.known[b*256+k] for k in range(256)]
            if any(v is None for v in blk):
                recs.append(None); continue
            X = ntt(blk[:])                                    # (-1)^b·cs1 → (-1)^b·ĉ∘ŝ1
            recs.append([(_modq(X[k]) * chatinv[k]) % Q for k in range(256)])
        out = {'coverage': self.coverage(), 's1ntt': recs}
        if true_s1ntt is not None and all(r is not None for r in recs):
            out['agreement'], out['sign'] = agreement(recs, true_s1ntt)
        return out
