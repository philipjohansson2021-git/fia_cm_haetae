...
/* 축 A(HW 글리치) 트리거 지점 선택: 호스트 'T' 로 g_trig_line 설정.
   -1  = 전체 c·s~+y 구간(기존 호환, 기본값)
   FL_*= 그 연산 "직전(HI)·직후(LO)"에만 트리거 → ext_offset≈0 으로 그 구간만 정밀 타격.
   (FAULT_SIM 빌드 전용. 일반 빌드는 기존처럼 c·s~+y 전체를 감싼다.) */
#ifdef FAULT_SIM
volatile int g_trig_line = -1;
#define TRIG_HI(pt)     do { if (g_trig_line == (pt)) trigger_high(); } while (0)
#define TRIG_LO(pt)     do { if (g_trig_line == (pt)) trigger_low();  } while (0)
#define TRIG_BROAD_HI() do { if (g_trig_line <  0)    trigger_high(); } while (0)
#define TRIG_BROAD_LO() do { if (g_trig_line <  0)    trigger_low();  } while (0)
#else
#define TRIG_HI(pt)     ((void)0)
#define TRIG_LO(pt)     ((void)0)
#define TRIG_BROAD_HI() trigger_high()
#define TRIG_BROAD_LO() trigger_low()
#endif
...
#ifdef AXISA_JIG
/* ===== 축 A fast-jig (G3-0): +y 구간만 반복 재실행하여 스윕 가속 (서명 8s → 수십 ms) =====
   호스트 'J' 0 = prime(정상 서명 1회로 채택 시도의 y·cs·golden z 를 저장), 'J' 1 = fire(+y 재실행;
   글리치 표적). 산술은 빌드 VARIANT 와 동일(fused / twopass). 찾은 파라미터는 반드시 full-sign 'p' 로
...
#if defined(HAETAE_VARIANT_TWOPASS)
  for (i = 0; i < HAETAE_L; ++i) for (int _k = 0; _k < HAETAE_N; _k++)
    z1.vec[i].coeffs[_k] = HAETAE_LN * j_cs1.vec[i].coeffs[_k];
  for (i = 0; i < HAETAE_K; ++i) for (int _k = 0; _k < HAETAE_N; _k++)
    z2.vec[i].coeffs[_k] = HAETAE_LN * j_cs2.vec[i].coeffs[_k];
  trigger_high();
  for (i = 0; i < HAETAE_L; ++i) for (int _k = 0; _k < HAETAE_N; _k++)
    z1.vec[i].coeffs[_k] += j_y1.vec[i].coeffs[_k];
  for (i = 0; i < HAETAE_K; ++i) for (int _k = 0; _k < HAETAE_N; _k++)
    z2.vec[i].coeffs[_k] += j_y2.vec[i].coeffs[_k];
  trigger_low();
#else
  trigger_high();
  polyfixvecl_add(&z1, &j_y1, &j_cs1);     // 융합 +y (fused 빌드)
  polyfixveck_add(&z2, &j_y2, &j_cs2);
  trigger_low();
#endif
...
static void infect_sig(uint8_t *sig, uint64_t delta, const uint8_t *sk, const uint8_t *mu){
  uint8_t factor = (uint8_t)(0u - (uint8_t)((delta | (0u - delta)) >> 63)); // delta!=0 -> 0xFF
  uint8_t seed[HAETAE_SEEDBYTES + 8 + HAETAE_CRHBYTES]; uint8_t ks[HAETAE_CRYPTO_BYTES];
  ...
  for (int i = 0; i < HAETAE_CRHBYTES; i++) seed[HAETAE_SEEDBYTES + 8 + i] = mu[i]; // 메시지 결합(마스크 신선도)
  shake256(ks, HAETAE_CRYPTO_BYTES, seed, HAETAE_SEEDBYTES + 8 + HAETAE_CRHBYTES);
...
reject:
...
#endif
  TRIG_LO(FL_CS);                          // T1 정밀 트리거: c·s 직후
  polyvecl_cneg(&cs1, b & 1);
  polyveck_cneg(&cs2, b & 1);
  /* 3-2. z1 = y1 + (-1)^b·c·s1  (T2 = +y 덧셈) — 실제 full-HAETAE 연산 그대로.
     축 A(HW 클럭글리치)는 아래 +y 루프의 실연산을 공격한다(인위적 가드/계측 없음). 축 B(FAULT_SIM)는
     fault_fire 로 +y 스킵을 모델링. VARIANT=twopass 는 융합 대신 비융합 2-pass 로 계산하는 인과
     대조군(G3-4)으로, +y 를 독립 루프로 분리해 loop-abort 로 물리 clean T2 가 실현되는지 확인한다
     (무결함 시 z1[i]=LN·cs1[i]+y1[i] 로 융합과 산술 동일 → GOLDEN 보존). */
#if defined(HAETAE_VARIANT_TWOPASS)
  /* pass 1: z = LN·cs  (clean-T2 값이 z 에 단독으로 저장되는 순간이 존재) — 트리거 밖 */
  for (i = 0; i < HAETAE_L; ++i)
    for (int _k = 0; _k < HAETAE_N; _k++)
      z1.vec[i].coeffs[_k] = HAETAE_LN * cs1.vec[i].coeffs[_k];
  { unsigned int kk; for (kk = 0; kk < HAETAE_K; ++kk)
      for (int _k = 0; _k < HAETAE_N; _k++)
        z2.vec[kk].coeffs[_k] = HAETAE_LN * cs2.vec[kk].coeffs[_k]; }
  TRIG_HI(FL_ADDY);                        // +y 독립 루프만 트리거로 감쌈 (loop-abort 표적)
#ifdef FAULT_SIM
  if (!fault_fire(FL_ADDY))                // 축 B: +y 루프 스킵 모델 → z=LN·cs=clean T2 (else 루프 생략)
#endif
  { for (i = 0; i < HAETAE_L; ++i)
      for (int _k = 0; _k < HAETAE_N; _k++)
        z1.vec[i].coeffs[_k] += y1.vec[i].coeffs[_k];
    { unsigned int kk; for (kk = 0; kk < HAETAE_K; ++kk)
        for (int _k = 0; _k < HAETAE_N; _k++)
          z2.vec[kk].coeffs[_k] += y2.vec[kk].coeffs[_k]; } }
  TRIG_LO(FL_ADDY);
#else
  TRIG_HI(FL_ADDY);                        // T2 정밀 트리거: +y 덧셈 직전
#ifdef FAULT_SIM
  if (fault_fire(FL_ADDY)) {               // T2(축 B/SW 모델): +y 스킵 → z=(-1)^b·c·s (y 제거, s1 유출)
...
#endif
  { polyfixvecl_add(&z1, &y1, &cs1);       // 실제 융합 +y (축 A HW 글리치가 이 실연산을 교란)
    polyfixveck_add(&z2, &y2, &cs2); }
  TRIG_LO(FL_ADDY);                        // T2 정밀 트리거: +y 덧셈 직후
#endif
  TRIG_BROAD_LO();

  /* 4. 거부 샘플링(서명경계 B1 / 바이모달 B0) */
  TRIG_HI(FL_REJECT);                          // RB 정밀 트리거: 거부 판정 계산 직전
  ...
  reject2 &= 1;
  reject2 &= (b & 0x2) >> 1;
  TRIG_LO(FL_REJECT);                          // RB 정밀 트리거: 거부 판정 계산 직후
#ifdef FAULT_SIM
...
#if defined(HAETAE_VARIANT_IRV) || defined(HAETAE_VARIANT_LEEHA)
  infect_sig(sig, cm_delta, sk, mu);   // M4(무분기 감염): 결함이면 출력 난수화 → 키유출 0
#endif
  return 0;
}
