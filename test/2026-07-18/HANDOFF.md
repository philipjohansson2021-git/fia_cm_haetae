...
**정직성 3조건(성격 구분 — 논문/발표 필수)**:
- **① cs 사전 0화(`T1_CS_ZEROINIT`, 인과대조 빌드) = 실제 공격 성립 조건**(검증 편의 아님). **미수정 융합
  레퍼런스는 이게 없어 물리 T1 저항**(스킵=쓰레기). ⇒ "레퍼런스를 깼다"가 아니라 "구현이 깨끗한 스킵을
  허용하면 완결". 레퍼런스 안전은 우연(융합+미초기화). T2의 twopass 대조와 동형.
- **② 고정 nonce = T1 2-서명 출력 차분의 실제 요건**(두 서명 출력 y 동일; 결정론 타깃엔 현실적) + 랩 편의(DRBG·jig 재생).
  (T2 단일 서명 출력은 불필요.)
- **③ 참 s1('s' 스트림) 대조 = 순수 검증 편의**(랩 오라클; 공격 자체엔 불필요).
...
- **노트북**: `Lab_HAETAE_F4_T1_AxisA.ipynb`(22셀, 재현판 — 개선 완료). 구성: 0)빌드 1)부트스트랩 2)트리거+드라이버
  단계1(`validate_t1_chain_sw` 체인 768/768) 2(`run_t1_fullsign` baseline **저항=정상**) 3(`run_t1_jig` 밴드·33%)
  **4(`run_t1_accumulate` 누적 768/768 ★)** 결과확인 부록. intro에 **정직성 3조건** 명시. **사용자 재현 확인됨**
  (2026-07-18: 노트북 단계4에서 `accumulated_full=True`). `run_t1_accumulate` = `t1_auto.py` 자율판과 동일 로직,
  기본 ext_bands `(41400-41470)/(91300-92000)/(142100-142750)`(s1[0]가 254 등 부족 시 첫 밴드를 41,421로 좁힘).
...
- 글리치가 **한 반복(다항식 1개) 계산을 통째로 스킵** → cs 사전0화(EDIT1) 덕에 그 블록=0 → 2-서명 출력 차분
