...
reject:
  ...
  /* 3-2. z1 = y1 + (-1)^b·c·s1  (T2 = +y 덧셈) — 실제 full-HAETAE 연산 그대로.
     축 A(HW 클럭글리치)는 아래 polyfixvecl_add/polyfixveck_add 의 실제 +y 루프를 공격한다
     (인위적 가드/계측 없음 — full-HAETAE 그대로). 축 B(FAULT_SIM)만 fault_fire 로 +y 스킵을 모델링. */
...
#endif
  { polyfixvecl_add(&z1, &y1, &cs1);       // 실제 +y (축 A HW 글리치가 이 실연산을 교란)
